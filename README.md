...
### 4. GitHub OAuth App を作成

1. GitHub → **Settings** → **Developer settings** → **OAuth Apps** → **New OAuth App**
2. 以下を入力：
   - **Application name**: まだある？（任意）
   - **Homepage URL**: `http://localhost:3000`（ローカル用）
   - **Authorization callback URL**: `http://localhost:3000/api/auth/callback/github`
3. **Register application** をクリック
4. **Client ID** をコピー → `.env.local` の `GITHUB_ID` に貼り付け
5. **Generate a new client secret** → コピー → `.env.local` の `GITHUB_SECRET` に貼り付け
...
### 3. GitHub OAuth App のコールバック URL を追加

GitHub → OAuth App の設定 → **Authorization callback URL** に追加:
